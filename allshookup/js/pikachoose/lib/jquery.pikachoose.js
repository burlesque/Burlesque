...
(function($) {
	...
	$pc.fn.extend({
		...
		build: function() {
            ...
            //
            this.details    = $("<div class='stage_details'></div>").insertAfter(this.image);
            //
			...
			$('<img />').on('load',{data:data},function(){
				...
				self.hover(
					function(e){
						...
						that.tooltip.show().stop(true,true).html(data.caption).animate({top:$(this).parent().position().top+130, left:$(this).parent().position().left+5, opacity: 1.0},400);
					},
					function(e){
		...
		bindEvents: function() {
            ...
            this.image.on('swipeleft', {self:this},this.nextClick);
            this.image.on('swiperight', {self:this},this.prevClick);
		...
		imgClick: function(e,x) {
            $('html, body').animate({
                scrollTop: $("#group_type").offset().top+3
            }, 800);

            var details = $(this.nextSibling.nextSibling);
            $('.stage_details').html(details.html());
